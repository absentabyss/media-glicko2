...
class ImageRankerApp:
    ...
    def _set_media_on_label(self, path: Path, widget: tk.Label, side: str) -> None:
        ...
        # Image / GIF path.  Stop VLC so it ceases painting to the HWND;
        # pause() leaves the last frame rendered and the image label can't
        # appear on top of VLC's native Win32 child window.  _stop_vlc is
        # async (background thread) so the main thread never blocks.
        self._stop_vlc(side)
